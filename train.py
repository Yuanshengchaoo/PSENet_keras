...
from loss_metric import locate_loss, locate_metric, score_loss, score_metric, siam_loss
...
def sanitize_fit_kwargs(model, fit_kwargs):
    """Remove any kwargs that are not accepted by the current Keras fit signature."""

    accepted = set(inspect.signature(model.fit).parameters)
    sanitized = {key: value for key, value in fit_kwargs.items() if key in accepted}
    dropped = sorted(set(fit_kwargs) - accepted)
    if dropped:
        print(f"Dropping unsupported fit kwargs: {dropped}")
    return sanitized
...
with strategy.scope():
    ...
    reduce_lr = callbacks.ReduceLROnPlateau(
        monitor='val_loss',
        factor=myModelConfig.learning_rate_decay_factor,
        patience=5,
        verbose=1,
        mode='min',
        cooldown=10,
...
myTensorboard = callbacks.TensorBoard(
    log_dir=myModelConfig.summary_dir,
    histogram_freq=0,
    write_graph=False,
    write_images=True,
)
...
try:
    ...
    if any(trigger in str(exc) for trigger in legacy_triggers):
        print("Retrying fit after removing deprecated multiprocessing kwargs…")
